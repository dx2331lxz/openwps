...
从 git 历史看，openwps 的能力已经不只是“AI 改格式”，还包含一组完整的文档工程功能：
...
README 中的关键描述来自当前代码实现，而不是产品设想。

| 能力 | 代码入口 |
| --- | --- |
| 工具元数据、权限、执行位置、延迟加载 | `server/app/tool_registry.py` |
| 所有 AI 执行计划进入服务端执行 | `server/app/ai.py` 的 `_split_execution_plan()` |
| ReAct 会话、停止条件、工具计划、上下文回灌 | `server/app/ai.py` |
| 持久 run 网关、事件流、取消、按会话查询 active run | `server/app/ai.py`、`server/app/factory.py` |
| 后端可执行文档工具清单 | `server/app/doc_sessions.py` 的 `SERVER_EXECUTABLE_DOCUMENT_TOOLS` |
| 文档工具执行、版本事件、workspace 写回 | `server/app/doc_sessions.py` |
| Node worker 文档工具、Mermaid 渲染、页面测量、截图 | `src/shared/document/tools.ts` |
| DOCX / Markdown 导入导出与样式解析 | `src/docx/`、`src/markdown/`、`server/node/document_io_worker.ts` |
| 模板库和 AI 模板分析 | `src/components/TemplateManagerModal.tsx`、`server/app/template_analysis.py` |
| 批注审阅能力 | `src/components/AddCommentDialog.tsx`、`src/components/CommentSidebar.tsx`、`src/shared/document/schema.ts` |
| AI 伴写多候选补全 | `src/components/Editor.tsx`、`server/app/ai.py` 的 `/api/ai/complete` |
| Slash / OCR 快捷入口 | `src/components/AISidebar.tsx`、`server/app/ai.py` |
| 多提供商、模型发现、OCR/视觉/Tavily 配置 | `server/app/config.py`、`src/components/SettingsModal.tsx` |
| workspace、`_references/`、`.openwps/memory/`、manifest、记忆写入 | `server/app/workspace.py` |
| 后端注入 workspace manifest，不信任前端作为事实来源 | `server/app/ai.py` 的 `_with_backend_workspace_docs()` 和工具后 delta 刷新 |
| 内置子 Agent 与只读边界 | `server/app/agents.py` |
| system prompt、tool schema hash、prompt cache | `server/app/content.py` |
| 上下文 delta 与压缩恢复附件机制 | `server/app/delta_injection.py` |
| Tool Harness 时间线说明 | `docs/TOOL_HARNESS_TIMELINE.md` |
